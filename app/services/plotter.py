...
def plot_menu_total(df: pd.DataFrame) -> Figure:
    ...
    df = df.nlargest(15, "total_qty")  # 상위 15개만 표시
    rects = ax.bar(df["menu_name"], df["total_qty"])
    _annot_bar(ax, rects)
...
def plot_menu_gender(df: pd.DataFrame) -> Figure:
    ...
    pivot = pivot.nlargest(10, pivot.columns.tolist(), keep='all')  # 주문량이 많은 10개 메뉴
...
def plot_hour_menu(df: pd.DataFrame) -> Figure:
    ...
    pivot = df.pivot_table(index="hour", columns="menu_name", values="qty_sum", aggfunc="sum").fillna(0)
    top5_menus = df.groupby('menu_name')['qty_sum'].sum().nlargest(5).index
    pivot = pivot[top5_menus]
    ...
    _theme(ax)
    ax.legend(title="Top 5 메뉴", bbox_to_anchor=(1.02, 1), loc='upper left')
    return fig
